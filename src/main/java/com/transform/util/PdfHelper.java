...
public class PdfHelper {
	...
	public static ITextRenderer getRender() throws DocumentException, IOException {
		...
		render.getFontResolver().addFont(path + "pdf/simsun.ttc", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);// 宋体
		render.getFontResolver().addFont(path + "pdf/MicrosoftYaHei.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
		render.getFontResolver().addFont(path + "pdf/HYCuSongJ.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);// 宋体加粗
		render.getFontResolver().addFont(path + "pdf/Wingdings 2.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);// 特殊字符
	...
	private static Configuration getFreemarkerConfig(String templatePath) throws IOException {
		freemarker.template.Version version = new freemarker.template.Version("2.3.28");
